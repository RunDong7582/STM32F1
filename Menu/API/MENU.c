...
int menu_command_callback(enum _menu_command command, ...)
{
    ...
    switch (command)
    {
        ...
        case SHOW_STRING: // 参数:( int16_t x, int16_t y, char *str ); 返回: uint8_t 字符串长度;
        {
            ...
            /* 按需使用参数 */
            ssd1306_SetCursor(show_x , show_y);
            retval = (int)ssd1306_WriteString(show_string, Font_6x8, White);
        ...
        case SHOW_CURSOR: // 参数:( int xsta, int ysta, int xend, int yend ); 返回: 无;
        {
            ...
            /* 按需使用参数 */
            ssd1306_InvertRectangle(cursor_xsta, cursor_ysta, cursor_xend, cursor_yend);
            break;
        }
        ...
        case DRAW_FRAME: // 参数:( int xsta, int ysta, int wide, int high ); 返回: 无;
        {
            ...
            /* 按需使用参数 */
            ssd1306_DrawRectangle(frame_x, frame_y, frame_x + frame_width,
                            frame_y + frame_height, Black);
            break;
        }

        /* Input */
        case GET_EVENT_ENTER: // 参数: 无; 返回: 布尔;
            retval = KEY.Enter + KEY.Right;
            break;

        case GET_EVENT_BACK: // 参数: 无; 返回: 布尔;
            retval = KEY.Back + KEY.Left;
            break;

        case GET_EVENT_WHEEL: // 参数: 无; 返回: int16_t 滚动量;
            if (KEY.Up)         {KEY.Up = 0;   retval = 1 ;}
            else if (KEY.Down)  {KEY.Down = 0; retval = -1;}
